...
**IA:** GitHub Copilot  
**Status:** ✅ CONCLUÍDA  
**Resumo:** Implementado serviço de análise automática de petições iniciais usando LLM (GPT-4) para sugerir documentos relevantes necessários para análise jurídica completa. Este é o segundo passo do fluxo de análise de petição inicial (FASE 7). **Principais entregas:** (1) **Novo serviço completo: `ServicoAnaliseDocumentosRelevantes` (860 linhas)** - 4 exceções customizadas (ErroAnaliseDocumentosRelevantes, ErroPeticaoNaoEncontrada, ErroDocumentoPeticaoNaoEncontrado, ErroParsingRespostaLLM), constantes de configuração (modelo GPT-4, temperatura 0.3, timeout 60s, 5 chunks RAG), prompt engineering robusto com formato JSON estruturado (PROMPT_SISTEMA_ANALISE_DOCUMENTOS de 200 linhas), função construir_prompt_analise_peticao() que combina texto petição + contexto RAG, classe principal com método analisar_peticao_e_sugerir_documentos() em 6 etapas (validar petição, recuperar texto do ChromaDB, busca RAG, chamar LLM, parsear JSON, atualizar estado), 6 métodos auxiliares privados para cada etapa, validações completas (petição existe, documento no ChromaDB, JSON válido, estrutura correta, prioridades válidas), tratamento de erros robusto (busca RAG não-crítica, documento individual inválido não falha análise toda); (2) **Nova função em `servico_banco_vetorial.py`: obter_documento_por_id() (110 linhas)** - busca chunks filtrando por documento_id, ordena por chunk_index para manter ordem original, retorna estrutura com documents/metadatas/ids/count, trata caso documento não encontrado com estrutura vazia; (3) **Novo endpoint: POST /api/peticoes/{peticao_id}/analisar-documentos (200 linhas)** - valida petição existe (404 se não), verifica re-análise (retorna resultado anterior se já analisado), agenda análise em background (BackgroundTasks), retorna 202 Accepted imediatamente, função executar_analise_em_background() assíncrona executada após response, trata 5 tipos de erros possíveis com registro no gerenciador, logging detalhado em cada etapa; (4) **Documentação completa em `ARQUITETURA.md` (+120 linhas)** - novo endpoint documentado com descrição, contexto de negócio, padrão assíncrono, exemplos JSON, prompt da LLM explicado, prioridades definidas (essencial/importante/desejavel), exemplos de documentos típicos, integração ChromaDB, fluxo UX frontend, tratamento de erros background. **Decisões técnicas:** (1) Modelo GPT-4 (não 3.5) - melhor raciocínio jurídico, (2) Temperatura 0.3 baixa - respostas factuais consistentes, (3) Formato JSON estruturado - facilita parsing e validação, (4) Truncamento 8k caracteres - respeita limite tokens GPT-4, (5) Busca RAG não-crítica - continua sem contexto se falhar, (6) Validação prioridade permissiva - usa padrão se inválida, (7) Continuar se documento individual inválido - 9 válidos melhor que 0, (8) Processamento assíncrono - análise pode demorar 10-60s. **Fluxo completo:** Cliente POST → valida petição → agenda background → retorna 202 → background: recupera texto ChromaDB → busca RAG → constrói prompt → chama GPT-4 → parseia JSON → atualiza petição → cliente polling status → documentos_sugeridos preenchido. **PRÓXIMA TAREFA:** TAREFA-043 (Backend - Endpoint de Upload de Documentos Complementares). **MARCO:** 🎉 ANÁLISE DE DOCUMENTOS RELEVANTES IMPLEMENTADA! Sistema agora analisa petições automaticamente e sugere documentos usando GPT-4, integração perfeita ChromaDB + LLM + gerenciador estado.
